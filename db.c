...
db_open(const char *pathname, int oflag, ...)
{
    ...
    strcat(db->name, ".idx");  /*data文件后缀 .dat, index文件后缀 .idx*/
    ...
    if ((oflag & (O_CREAT | O_TRUNC))  == (O_CREAT | O_TRUNC)){
        ...
        if (statbuff.st_size == 0){
            ...
            while ((i!=0) && (writen=write(db->idxfd, ptr, i)) !=0){
                if (writen == -1){
                    if (errno == EINTR)
                        continue;
                    else{
                        printf ("%s: write index file error\n", __FUNCTION__);
                        exit(1);
                    }
                 }
...
/*@ Allocate and initialize a DB structure and it's buffers
 *@
 *@
 */
...
/*@ _db_free:
 *@         free a DB structure and any other malloc'ed buffers.
 *@         and fd.
 *@
 */
...
/*@ db_fetch:
 *@     fetch a record specificed by key. Return
 *@     a pointer to the null-terminated data
 */
...
db_fetch(DBHANDLE h, const char *key)
{
    ...
    if (un_lock(db->idxfd, db->chainoff, SEEK_SET, 1)<0)
        dberrlog(elog, "db_fetch: unlock error");
...
_db_find_and_lock(DB *db, const char *key, int writelock)
{
    ...
    if (writelock){
        ...
        if (readw_lock(db->idxfd, db->chainoff, SEEK_SET, 1)<0)
            dberrlog(elog, "_db_find_and_lock: readw_lock error");
    }
...
_db_readptr(DB *db, off_t offset)
{
    ...
    if (lseek(db->idxfd, offset, SEEK_SET) == -1)
        dberrlog(elog, "_db_readptr: lseek error to ptr field");
    if (read(db->idxfd, asciiptr, PTR_SZ) != PTR_SZ)
        dberrlog(elog, "_db_readptr: read error of ptr field");
    asciiptr[PTR_SZ] = '\0';
    return atol(asciiptr);
}

/*@ _db_readidx:
 *@    offset is the index to be searched.Through this offset, we get
...
_db_readidx(DB *db, off_t offset)
{
    ...
    if ((db->idxoff = lseek(db->idxfd, offset, offset==0 ? SEEK_CUR : SEEK_SET)) == -1)
        dberrlog(elog, "_db_readidx: lseek error");
    ...
    if ((i=readv(db->idxfd, &iov[0], 2)) != PTR_SZ + IDXLEN_SZ) {
        ...
        dberrlog(elog, "_db_readidx: readv error of index record");
    }
    ...
    if ((db->idxlen = atoi(asciilen) < IDXLEN_MIN) || 
        ...
        dberrlog(elog, "_db_readidx: invalid length");

    if((i=read(db->idxfd, db->idxbuf, db->idxlen)) != db->idxlen)
        dberrlog(elog, "_db_readidx: missing first separator");

    if (db->idxbuf[db->idxlen-1] != NEWLINE)
        dberrlog(elog, "_db_readidx: missing newline");
    ...
    *ptr1++ = '\0';
    ...
    if ((ptr2 = strchr(ptr1, SEP)) != NULL)
        dberrlog(elog, "_db_readidx: too many separators");


    if ((db->datoff = atol(ptr1)) < 0)
        dberrlog(elog, "_db_readidx: starting offset < 0");
    if ((db->datlen = atol(ptr2)) <= 0 || db->datlen > DATLEN_MAX)
        dberrlog(elog, "_db_readdix: invalid length");
...
_db_readdat(DB *db)
{
    if (lseek(db->datfd, db->datoff, SEEK_SET) == -1)
        dberrlog(elog, "_db_readdat: lseek error");
    if (read(db->datfd, db->datbuf, db->datlen) != db->datlen)
        dberrlog(elog, "_db_readdat: read error");
    if (db->datbuf[db->datlen-1] != NEWLINE)
        dberrlog(elog, "_db_readat: missing newling");
...
db_delete(DBHANDLE h, const char *key)
{
	...
	if (un_lock(db->idxfd, db->chainoff, SEEK_SET, 1) < 0)
		dberrlog(elog, "db_delete: un_lock error");
	return(rc);
}
...
_db_dodelete(DB *db)
{
	...
	*ptr = 0;	/* null terminate for _db_writedat */
	...
	if (writew_lock(db->idxfd, FREE_OFF, SEEK_SET, 1) < 0)
        dberrlog(elog, "_db_dodelete: writew_lock error");
	...
	if (un_lock(db->idxfd, FREE_OFF, SEEK_SET, 1) < 0)
		dberrlog(elog, "_db_dodelete: un_lock error");
}
...
_db_writedat(DB *db, const char *data, off_t offset, int whence)
{
	...
	if (whence == SEEK_END) /* we're appending, lock entire file */
		if (writew_lock(db->datfd, 0, SEEK_SET, 0) < 0)
			dberrlog(elog, "_db_writedat: writew_lock error");

	if ((db->datoff = lseek(db->datfd, offset, whence)) == -1)
		dberrlog(elog, "_db_writedat: lseek error");
	...
	if (writev(db->datfd, &iov[0], 2) != db->datlen)
		dberrlog(elog, "_db_writedat: writev error of data record");

	if (whence == SEEK_END)
		if (un_lock(db->datfd, 0, SEEK_SET, 0) < 0)
			dberrlog(elog, "_db_writedat: un_lock error");
}
             ...
             off_t offset, int whence, off_t ptrval)
{
	...
	if ((db->ptrval = ptrval) < 0 || ptrval > PTR_MAX){
        dberrlog(elog, "_db_writeidx: invalid");
        exit(1);
    }
	...
	if (len < IDXLEN_MIN || len > IDXLEN_MAX)
		dberrlog(elog, "_db_writeidx: invalid length");
	...
	if (whence == SEEK_END)		/* we're appending */
		if (writew_lock(db->idxfd, ((db->nhash+1)*PTR_SZ)+1,
		  SEEK_SET, 0) < 0)
			dberrlog(elog, "_db_writeidx: writew_lock error");

	/*
	 * Position the index file and record the offset.
	 */
	if ((db->idxoff = lseek(db->idxfd, offset, whence)) == -1)
		dberrlog(elog, "_db_writeidx: lseek error");
	...
	if (writev(db->idxfd, &iov[0], 2) != PTR_SZ + IDXLEN_SZ + len)
		dberrlog(elog, "_db_writeidx: writev error of index record");

	if (whence == SEEK_END)
		if (un_lock(db->idxfd, ((db->nhash+1)*PTR_SZ)+1,
		  SEEK_SET, 0) < 0)
			dberrlog(elog, "_db_writeidx: un_lock error");
}
...
_db_writeptr(DB *db, off_t offset, off_t ptrval)
{
	...
	if (ptrval < 0 || ptrval > PTR_MAX){
        dberrlog(elog, "_db_writeptr: invalid ptr");
        exit(1);
    }
	sprintf(asciiptr, "%*lld", PTR_SZ, (long long)ptrval);

	if (lseek(db->idxfd, offset, SEEK_SET) == -1)
		dberrlog(elog, "_db_writeptr: lseek error to ptr field");
	if (write(db->idxfd, asciiptr, PTR_SZ) != PTR_SZ)
		dberrlog(elog, "_db_writeptr: write error of ptr field");
}
...
db_store(DBHANDLE h, const char *key, const char *data, int flag)
{
	...
	if (datlen < DATLEN_MIN || datlen > DATLEN_MAX)
		dberrlog(elog, "db_store: invalid data length");
...
doreturn:	/* unlock hash chain locked by _db_find_and_lock */
	if (un_lock(db->idxfd, db->chainoff, SEEK_SET, 1) < 0)
		dberrlog(elog, "db_store: un_lock error");
	return(rc);
}
...
_db_findfree(DB *db, int keylen, int datlen)
{
	 ...
	 */
	if (writew_lock(db->idxfd, FREE_OFF, SEEK_SET, 1) < 0)
		dberrlog(elog, "_db_findfree: writew_lock error");
	 ...
	 */
	if (un_lock(db->idxfd, FREE_OFF, SEEK_SET, 1) < 0)
		dberrlog(elog, "_db_findfree: un_lock error");
	return(rc);
}
...
db_rewind(DBHANDLE h)
{
	...
	if ((db->idxoff = lseek(db->idxfd, offset+1, SEEK_SET)) == -1)
		dberrlog(elog, "db_rewind: lseek error");
}
...
db_nextrec(DBHANDLE h, char *key)
{
	...
	if (readw_lock(db->idxfd, FREE_OFF, SEEK_SET, 1) < 0)
		dberrlog(elog, "db_nextrec: readw_lock error");
...
doreturn:
	if (un_lock(db->idxfd, FREE_OFF, SEEK_SET, 1) < 0)
		dberrlog(elog, "db_nextrec: un_lock error");
	return(ptr);
}
